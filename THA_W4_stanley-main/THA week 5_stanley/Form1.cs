...
namespace THA_week_5_stanley
{
    public partial class Form1 : Form
    {
        ...
        private void remove_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (teams[i].Teamname == comboBox2.Text)
                {
                    if (teams[i].Players.Count <= 11)
                    {
                        MessageBox.Show("Eror, jumlah gada 11");
                    }
                    else
                    {
                        for (int j = 0; j < teams[i].Players.Count; j++)
                        {
                            if (listBox1.Text == "(" + teams[i].Players[j].Playernum + ") " + teams[i].Players[j].Playername + ", " + teams[i].Players[j].Playerpos)
                            {
